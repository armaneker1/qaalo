...
$.extend(MaxLength.prototype, {
	...
	propertyName: 'maxlength',
        urlRegex: new RegExp(/[-a-zA-Z0-9@:%_\+.~#?&//=]{1,256}\.[a-z]{2,4}\b(\/[-a-zA-Z0-9@:%_\+.~#?&//=]*)?/gi),
	...
	_attachPlugin: function(target, options) {
		...
		target.addClass(this.markerClassName).
			...
			bind('keypress.' + this.propertyName, function(event) {
				...
				return (event.ctrlKey || event.metaKey || ch == '\u0000' || plugin._getLength($(this)) < inst.options.max);
			}).
	...
	_optionPlugin: function(target, options, value) {
		...
		var inst = target.data(this.propertyName);
                options.max = parseInt(target.attr("charlength"));
		...
		if (inst.options.showFeedback == 'active') { // Additional event handlers
                                target.bind('focus.' + this.propertyName, function() {
					inst.focussed = true;
					inst.feedbackTarget.show('fast');
				}).bind('blur.' + this.propertyName, function() {
					inst.focussed = false;
					inst.feedbackTarget.hide('fast');
				});
			inst.feedbackTarget.css('display', 'none');
		}
	...
	_curLengthPlugin: function(target) {
        ...
        _getLength: function(target) {
            return target.val().replace(/\r\n/g, '~~').replace(/\n/g, '~~').replace(target.attr("placeholder"),"").replace(this.urlRegex,"~~~~~~~~~~~~~~~~~~~~").length;
        },
	...
	_checkLength: function(target) {
		var inst = target.data(this.propertyName);
		var value = target.val();
		var len = this._getLength(target);
		
		if (len > inst.options.max && inst.options.truncate) { // Truncation
                    target.val(value.substring(0, inst.options.max));
		}
		
                inst.feedbackTarget.toggleClass(this._fullClass, len >= inst.options.max).toggleClass(this._overflowClass, len > inst.options.max);
                inst.feedbackTarget.toggleClass(this._fullClass, len >= inst.options.max).toggleClass(this._overflowClass, len > inst.options.max);
